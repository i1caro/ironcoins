...
class Engine(object):
    ...
    def __init__(self, movement_matrix):
        movement_matrix.sort()
        # self.timetable = [list() for t in self.TIME]
        self.create_steam_line_movement(movement_matrix)

    def create_steam_line_movement(self, actions):
        for action in actions:
            self.timetable[action.initiative].append(action)

    def start(self):
        for action in self.timetable:
            unplayed = self.play_actions(action)
            try:
